    ...
    public function fix_wrong_parents(): void {
        ...
        foreach ($categoriestofix as $childcategoryid => $childcontextid) {
            ...
            if ($topcategoryid) {
                ...
                // Also all child categories must be changed to the new context id of our current category.
                $this->change_child_hierarchy($childcategoryid, $parentcontextid);
            }
        }
    }

    /**
     * Change all children of the current $parentid to the given $contextid. Do this
     * recursive to change children in the hierarchy.
     *
     * @param int $parentid;
     * @param int $contextid;
     */
    public function change_child_hierarchy(int $parentid, int $contextid) {
        global $DB;
        $children = $DB->get_records_select(
            'question_categories', 'parent = ?', ['parent' => $parentid], '', 'id, contextid');
        foreach ($children as $child) {
            if ($child->contextid != $contextid) {
                $DB->set_field('question_categories', 'contextid', $contextid, ['id' => $child->id]);
                $this->change_child_hierarchy((int)$child->id, $contextid);
            }
        }
    }
