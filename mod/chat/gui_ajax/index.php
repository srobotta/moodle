...
// Get group settings in case these are used.
...
if (!$chatsid = chat_login_user($chat->id, 'ajax', $settings->groupid, $course)) {
...
$modulecfg = array(
    ...
    'chat_lasttime' => 0,
    'chat_lastrow' => null,
    'chatroom_name' => $courseshortname . ": " . format_string($chat->name, true) . $settings->groupname
);
$PAGE->requires->js_init_call('M.mod_chat_ajax.init', array($modulecfg), false, $module);

$PAGE->set_title(get_string('modulename', 'chat').": $courseshortname: ".format_string($chat->name, true)."$settings->groupname");
...
$table->data = [[
    '<div class="form-inline"><div class="d-flex"><label class="accesshide" for="input-message">'.
    ...
    get_string('themes').
    ' &raquo; </a></div></div>' .
    ($settings->readonly ? '<span class="hide readonly-group">' . get_string('readonly', 'chat') . '</span>' : '')
]];
