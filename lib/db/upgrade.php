...
privatefiles,moodle|/user/files.php';
    ...
    if ($oldversion < 2023011300.01) {

        // Define new field notifyemail for badge table.
        $table = new xmldb_table('badge');
        $field = new xmldb_field('notifyemail', XMLDB_TYPE_TEXT, null, null, null, null, null, 'notification');

        // Add new field notifyemail if it doesn't exist yet and prefill it with
        // the email address of the badge creator in case notification is set for this badge.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);

            // For all badges, when the creator should be informed if a badge is awarded to a student, then put the email address
            // of the creator in the notifyemail field. The "notification" column triggers the frequency of the notifications.
            $sql = '
                SELECT b.id, u.email from {badge} b
                JOIN {user} u ON b.usercreated = u.id
                WHERE b.notification > 0
            ';
            // The where clause should actually use BADGE_MESSAGE_NEVER from the badgeslib.php, but I prefer not to
            // include the whole lib for just this constant (it shouldn't change anyway).
            foreach ($DB->get_records_sql($sql) as $row) {
                if (!empty($row->email)) {
                    $row->notifyemail = $row->email;
                    unset($row->email);
                    $DB->update_record('badge', $row);
                }
            }
        }

        // Main savepoint reached.
        upgrade_main_savepoint(true, 2023011300.01);
    }
