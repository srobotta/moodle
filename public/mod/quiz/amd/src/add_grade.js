...
const saveGrade = async(input, attempt, slot, mark) => {
    ...
    Ajax.call([request])[0]
        .then(async(res) => {
            if (validGradingStates.includes(res.state)) {
                ...
                if (p) {
                    ...
                    const classAttr = p.className;
                    const table = p.closest('table');
                    const avgCell = table.querySelector(`td.${classAttr.replace(/ +/g, '.')} > .avgcell`);
                    if (avgCell && res.average !== null && res.count !== null) {
                        avgCell.innerHTML = `<span class="average">${res.average}</span> `
                            + `<span class="count">(${res.count})</span>`;
                    }
                    // Add a notice above the chart to indicate that it must be updated.
                    const graph = document.querySelector('.graph > .chart-area');
                    if (graph && !graph.querySelector('.chart-needs-update')) {
                        const notice = document.createElement('div');
                        notice.className = 'chart-needs-update alert alert-info';
                        notice.setAttribute('role', 'status');
                        notice.textContent = await getString('gradeschangedreload', 'mod_quiz');
                        graph.insertBefore(notice, graph.firstChild);
                    }
                    return true;
                }
            }
        ...
        .fail(async(ex) => {
            const headline = await getString('error', 'core');
