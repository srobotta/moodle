...
class add_grade extends external_api {
    ...
    public static function execute(int $attempt, int $slot, string $mark): array {
        ...
        // Check whether the user may add grades.
        require_capability('mod/quiz:grade', $attemptobj->get_context());
        ...
        // Compute the average grade for this question and the number of graded responses it is
        // averaged over, using the same source as the overview report's average row. The average is
        // rescaled to the quiz's maximum grade (via quiz_rescale_grade) rather than the sum of the
        // question max marks, so it is consistent with the other cells in the table.
        $quiz = $attemptobj->get_quiz();
        $dm = new \question_engine_data_mapper();
        $qubaids = new qubaids_for_quiz($attemptobj->get_quizid(), false, true);
        $averages = $dm->load_average_marks($qubaids, [$slot]);
        $record = $averages[$slot] ?? null;
        if ($record && $maxmark > 0) {
            $average = quiz_rescale_grade($record->averagefraction * $maxmark, $quiz, 'question');
            $count = (int) $record->numaveraged;
        } else {
            $average = null;
            $count = 0;
        }
    ...
    protected static function render_result(string $data, int $slot, quiz_attempt $attempt) {
        ...
        $rawmark = $attempt->get_question_mark($slot);
        $quiz = $attempt->get_quiz();
        $mark = quiz_rescale_grade($rawmark, $quiz, 'question'); // Scaled to $quiz->grade, formatted.
        switch ((string)$state) {
            case 'mangrwrong':
            case 'mangrpartial':
            case 'mangrright':
            ...
            case 'gradedwrong':
            case 'gradedright':
                $fraction = $rawmark / $attempt->get_question_attempt($slot)->get_max_mark();
                break;
            default:
                $fraction = null;
        }
        ...
        if ($state->is_finished() && $state != \question_state::$needsgrading) {
            $feedbackimg = self::icon_for_fraction($fraction);
        }

        // Wrap the mark in a span carrying the state class so it is styled correctly.
        $link = \html_writer::tag('span', $feedbackimg . \html_writer::tag(
            'span',
            $mark,
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            ...
            'average' => new external_value(
                PARAM_RAW,
                'Average grade for this question, on the quiz grade scale',
                VALUE_OPTIONAL
            ),
            'count' => new external_value(PARAM_INT, 'Number of graded responses averaged', VALUE_REQUIRED),
        ]);
    }
}
